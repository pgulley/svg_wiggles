...
function nice_randwiggle(iterations, color, draw){
	this.draw = draw
	this.color = color

	this.width = draw.width()
	this.height = draw.height()/2

	this.M_x = 0 //first point x
	this.M_y = this.height // first point y
	...
	this.d = this.width/this.iterations // distance between inflection points
	this.max_r = this.d/2 //maximum radius of control points	
	...
	function inflection_point(Y, index, d, r){
 	...
 	this.string = function(){
 		return `M ${this.M_x} ${this.M_y} C ${this.C1_x} ${this.C1_y} ${this.points.map(function(p){return p.string()}).join()} ${this.width} ${this.M_y} ${this.width} ${this.M_y}`
 	}
 	...
 	this.path = this.draw.path(this.string())
 	this.path.fill('none')
	this.path.stroke({ color: this.color, width: 1, linecap: 'round', linejoin: 'round' })
	this.tl = this.path.timeline().persist(true)
...
function draw_nicewigs(layers, colors, durations){
	...
	itrange = [4,5,7,10]
	for(j = 0; j<layers; j++){
 		this.wiggles.push(new nice_randwiggle(random_choice(itrange), random_choice(colors), draw))
 	}
